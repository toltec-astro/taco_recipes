...
def get_transfer_func(nw, flo_Hz):
    ...
    lut_file = f"toltec{nw:d}/amps_correction_lut.csv"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--spacing", '-s', choices=['linear', 'log'], help='spacing of the tones.', default='linear')
    ...
    if option.spacing == 'log':
        def f_func(f0, f1, n):
            return np.logspace(np.log10(f0), np.log10(f1), n)
    elif option.spacing == 'linear':
        f_func = np.linspace
    else:
        raise ValueError('invalid spacing')
    if n_tones >= 2:
        tone_freqs_p = f_func((lo_value + lo_gap).to_value(u.Hz), (lo_value + lo_gap + freq_range).to_value(u.Hz), n_tones // 2) << u.Hz
        tone_powers_p = np.full(len(tone_freqs_p), option.tone_power_dbm_pos)
        tone_freqs_n = f_func((lo_value - lo_gap - freq_range).to_value(u.Hz), (lo_value - lo_gap).to_value(u.Hz), n_tones // 2) << u.Hz
    ...
    mask = np.isnan(tone_powers)
    tone_freqs = tone_freqs[~mask]
    tone_comb_freqs = tone_comb_freqs[~mask]
    tone_powers = tone_powers[~mask]
