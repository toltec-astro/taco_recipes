...
import matplotlib
import numpy as np
from astropy.table import Column, Table, join, vstack, join_distance
...
@timeit
def _match_d21(d21_0, d21_1, roi=None, max_shift=10 << u.MHz, return_locals=False):
...
def _match_tonelists(tl_0, tl_1, Qr_at_500MHz):
    ...
    return _match_d21(d21_0, d21_1, roi=None, return_locals=True)
...
def _get_tune_obsnum(data_rootpath, obsnum):
    ...
    for p in [
        ...
        f"toltec*[0-9]_{obsnum_str}_*.nc",
    ]:
    ...
    for f in files:
        with BasicObsData(files[0]).open() as bod:
            print(f"{f} {bod.meta['cal_obsnum']}")
            tune_obsnums.append(bod.meta["cal_obsnum"])
    tune_obsnum = max(tune_obsnums)
    logger.info(f"found {tune_obsnum=} for {obsnum=}")
    return tune_obsnum


def _get_kmp_files(data_rootpath, obsnum):
    f = _get_kmp_files_impl(data_rootpath, obsnum)
    if f is not None:
        return f
    tune_obsnum = _get_tune_obsnum(data_rootpath, obsnum)
    return _get_kmp_files_impl(data_rootpath, tune_obsnum)


def _get_kmp_files_impl(data_rootpath, tune_obsnum):
    files = []
    ...
    for p in [
        ...
        f"toltec_clipa/reduced/toltec*[0-9]_{obsnum_str}_*tune.txt",
        f"toltec_clipo/reduced/toltec*[0-9]_{obsnum_str}_*tune.txt",
        f"toltec*[0-9]_{obsnum_str}_*tune.txt",
    ]:
        files.extend(data_rootpath.glob(p))
    if not files:
        return None
    print(f"use kmp files: {files}")
...
def _make_matched_apt_nw(apt_left, apt_right, Qr_at_500MHz=None, debug_plot_kw=None, match_method="astropy"):
    ...
    _match1d = globals()[f"_match1d_{match_method}"]
    id_matched = _match1d(
    ...
    logger.info(f"matched n_good={np.sum(mgood)} n_bad={len(bad_matched)}")
    for e in bad_matched:
        li = mbad_idx[e["idx_left"]]
        ri = bad_idx[e["idx_right"]]
        id_matched["idx_right"][id_matched["idx_left"] == li] = ri
        id_matched["x_right"][id_matched["idx_left"] == li] = fr_right.to_value(u.Hz)[
            ri
        ]
    ...
    if dbk.get("f_shift_enabled", False):
        ...
        # plot shift data
        ax.plot(fs, adiqs0, color="C0", linestyle="-", label="f_in")
        ax.plot(fs, adiqs1, color="C1", linestyle="-", label="f_tone")
        ax.plot(
            fs + shift,
            adiqs0,
        ...
        ax.plot(
            shift_data["dfs"],
            shift_data["cross_correlation"],
            label=f'SNR={shift_data["shift_snr"]}',
        )
        max_shift = shift_data["max_shift"]
        ax.axvline(-max_shift.to_value(u.Hz), color="gray", linestyle="--")
        ax.axvline(max_shift.to_value(u.Hz), color="gray", linestyle="--")
        ax.axvline(0, color="gray", linestyle=":")
        ax.axvline(-shift.to_value(u.Hz), 0.9, 1, color="cyan", linewidth=2)
        ax.set_xlim(max_shift.to_value(u.Hz) * -3, max_shift.to_value(u.Hz) * 3)
        ax.legend()
        plt.show()
...
def _match1d_stilts(x_left, x_right, eps, join_type="left", fill_id_value=None):
    id_col = "idx"
    left = Table()
    ...
    right["x"] = x_right
    join_value = {"left": "all1", "right": "all2", "inner": "1and2", "outer": "1or2"}[
        join_type
    ]
    if len(x_left) == 0 or len(x_right) == 0:
        return None
    result = stilts_match1d(
        left,
        right,
        "x",
        eps,
...
def _match1d_astropy(x_left, x_right, eps, join_type="left", fill_id_value=None):
    id_col = "idx"
    left = Table()
    left[id_col] = np.arange(len(x_left))
    left["x"] = x_left
    ...
    result = join(
        left, right, keys="x", join_type="inner",
        join_funcs={
            "x": join_distance(eps)
        },
    ...
    # select best
    result["separation"] = np.abs(result["x_left"] - result["x_right"])
    result.sort("separation")
    _, u_left = np.unique(result["idx_left"], return_index=True)
    result = result[u_left]
    _, u_right = np.unique(result["idx_right"], return_index=True)
    result = result[u_right]
    if join_type == "left":
        result_left = Table()
        result_left["idx_left"] = left["idx"]
        result_left["x_left"] = left["x"]
        result_left = join(
            result_left,
            result[["idx_left", "idx_right", "x_right", "separation"]],
            keys="idx_left", join_type="left",
        )
        result = result_left
    if hasattr(result["idx_right"], "mask") and fill_id_value is not None:
        result["idx_right"] = result["idx_right"].filled(fill_id_value)
...
def _match_apt_worker(arg):
    logger = get_logger()
    apt_left, apt_right, nw, debug_plot_kw, match_method = arg
...
def make_matched_apt(apt_left, apt_right, debug_plot_kw=None, match_method="astropy", n_procs=4):
    logger = get_logger()
    ...
    with timeit("run matching"):
        ...
        if True:
            ...
            apt_matched = list(
                map_func(
                    _match_apt_worker,
                    [(apt_left, apt_right, nw, debug_plot_kw, match_method) for nw in nws],
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_rootpath", "-r", default="data_lmt", type=Path, help="Data root path."
    )
    ...
    parser.add_argument("--debug_plot", action="store_true")
    parser.add_argument("--method", default="astropy", choices=["astropy", "stilts"])
    ...
    apt_right = apt_in
    apt_matched = make_matched_apt(apt_left, apt_right, debug_plot_kw=debug_plot_kw, match_method=option.method)
    ...
    for c in apt_matched.colnames:
        if apt_matched[c].dtype == np.int64:
            ...
            if hasattr(apt_matched[c], "filled"):
                apt_matched[c] = apt_matched[c].filled(-1.0)
